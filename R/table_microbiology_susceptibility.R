#' MicrobiologySusceptibility table
#'
#' @description
#' R6 class for the CLIF microbiology_susceptibility table (antimicrobial susceptibility results).
#' Inherits all loading, validation and summary behaviour from [BaseTable].
#' Port of `clifpy.tables.microbiology_susceptibility.MicrobiologySusceptibility`.
#'
#' @export
#' @examples
#' \dontrun{
#' microbiology_susceptibility_table <- MicrobiologySusceptibility$new(
#'   data_directory = "data/clif",
#'   filetype = "parquet",
#'   timezone = "US/Central"
#' )
#' microbiology_susceptibility_table$validate()
#' }
MicrobiologySusceptibility <- R6::R6Class(
  classname = "MicrobiologySusceptibility",
  inherit = BaseTable,
  public = list(
    #' @description Create a MicrobiologySusceptibility table instance.
